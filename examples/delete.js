...
myKp.connect('sofia2.com', 1883)
	...
	.then(function(joinResponse) {
		...
		var ssapMessageDELETE = ssapMessageGenerator.generateDeleteWithQueryTypeMessage('delete from TestSensorTemperatura where Sensor.measure=24', 'TestSensorTemperatura', 'SQLLIKE', sessionKey);

		return myKp.send(ssapMessageDELETE);	
	})
	.then(function(deleteResponse) {
		var deleteResponseBody = JSON.parse(deleteResponse.body);
		
		if (deleteResponseBody.ok) {
			console.log('Executed delete for Ids: ' + deleteResponseBody.data);
		} else {
			throw new Error('Error executing delete: ' + deleteResponse.body);
		}
